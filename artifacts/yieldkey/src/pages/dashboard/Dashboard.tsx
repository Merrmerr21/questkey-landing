  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">My Properties</h1>
          <p className="text-muted-foreground mt-1">Manage and analyze your property pipeline.</p>
        </div>
        <Button onClick={() => setIsModalOpen(true)} size="lg" className="bg-primary hover:bg-primary/90 text-white shadow-sm">
      ...
      {!properties || properties.length === 0 ? (
        <motion.div 
          ...
          className="bg-white border border-dashed border-border rounded-3xl p-12 text-center flex flex-col items-center shadow-sm"
        >
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
          {properties.map((prop, i) => (
            <motion.div 
              ...
              className="group bg-white rounded-2xl border border-border overflow-hidden hover:border-primary/50 transition-all hover:shadow-md flex flex-col"
            >
              <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-4">
                  <div className="px-3 py-1 rounded-full bg-muted border border-border text-xs font-medium text-muted-foreground capitalize">
                ...
                <h3 className="text-xl font-bold text-foreground mb-2 line-clamp-1">{prop.address}</h3>
                ...
                <div className="flex gap-4 text-sm text-muted-foreground border-t border-border pt-4">
                  {prop.bedrooms && <div><span className="text-foreground font-medium">{prop.bedrooms}</span> Beds</div>}
                  {prop.bathrooms && <div><span className="text-foreground font-medium">{prop.bathrooms}</span> Baths</div>}
                  {prop.squareFeet && <div><span className="text-foreground font-medium">{prop.squareFeet}</span> Sqft</div>}
                </div>
              </div>
