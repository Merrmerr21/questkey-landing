...
export default function Agents() {
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-display font-bold text-white">Agent Roster</h1>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {AGENT_ROSTER.map((agent, i) => (
          <motion.div 
            ...
            className="group relative bg-card rounded-2xl border border-white/5 p-6 hover:border-white/20 hover:bg-white/[0.02] transition-all overflow-hidden"
          >
            ...
            <div className="flex justify-between items-start mb-6">
              <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg bg-gradient-to-br", agent.color)}>
                <agent.icon className="w-7 h-7" />
              </div>
              <div className="px-2.5 py-1 rounded-md bg-emerald-500/10 text-emerald-400 text-xs font-semibold">
                Available
              </div>
            </div>

            <h3 className="text-2xl font-bold font-display text-white mb-1">{agent.name}</h3>
            ...
            <div className="space-y-2 border-t border-white/5 pt-4">
              ...
              {[1,2,3].map(n => (
                <div key={n} className="flex items-center gap-2 text-sm text-white/70">
                  ...
                  <span className="flex-1 h-4 bg-white/5 rounded animate-pulse w-3/4" />
                </div>
              ))}
