...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans overflow-hidden">
      ...
      {/* 1. NAV */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 border-b ${scrolled ? 'bg-white/80 backdrop-blur-md border-border py-4 shadow-sm' : 'bg-transparent border-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-2 font-display font-bold text-xl tracking-tight text-foreground cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
          ...
          <button onClick={() => scrollTo('cta')} className="hidden sm:inline-flex px-5 py-2.5 rounded-full bg-primary hover:bg-primary/90 text-white text-sm font-medium transition-all hover:scale-105 active:scale-95 shadow-sm">
            Get Early Access
          </button>
        </div>
      </nav>
      ...
      {/* 2. HERO */}
      <section className="relative pt-40 pb-20 lg:pt-48 lg:pb-32 px-6 bg-white">
        <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-16 lg:gap-8 items-center">
          <motion.div 
            initial="hidden" animate="visible" variants={staggerContainer}
            className="max-w-2xl"
          >
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-xs font-medium text-primary mb-8">
            ...
            <motion.h1 variants={fadeUp} className="text-5xl sm:text-6xl lg:text-7xl font-display font-extrabold leading-[1.1] mb-6 text-foreground">
            ...
            <motion.div variants={fadeUp} className="bg-white border border-border shadow-sm rounded-2xl p-6 md:p-8 relative">
              <WaitlistForm />
      ...
      {/* 3. AI TEAM */}
      <section id="team" className="py-24 bg-muted border-y border-border relative px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-display font-bold text-foreground mb-4">Meet your investment team</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { name: "Miles", role: "Underwriting", desc: "Runs the numbers and compares strategy paths.", initials: "M", color: "from-blue-500 to-indigo-500" },
              { name: "Avery", role: "Acquisitions", desc: "Helps identify markets and acquisition fit.", initials: "A", color: "from-purple-500 to-fuchsia-500" },
              { name: "Rhea", role: "Risk", desc: "Flags downside risk, regulation, and weak assumptions.", initials: "R", color: "from-rose-500 to-orange-500" },
              { name: "Sloane", role: "Setup", desc: "Guides setup, furnishing, and guest-readiness.", initials: "S", color: "from-amber-400 to-yellow-500" },
              { name: "Kai", role: "Revenue", desc: "Optimizes pricing and revenue strategy.", initials: "K", color: "from-primary to-rose-400" },
              { name: "Theo", role: "Portfolio", desc: "Helps you think in portfolios, not one-off deals.", initials: "T", color: "from-cyan-500 to-blue-500" }
            ].map((agent, i) => (
              <motion.div 
                key={agent.name}
                initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }}
                variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { delay: i * 0.1 } } }}
                className="group p-6 rounded-2xl bg-white border border-border hover:border-primary/30 transition-all hover:shadow-md"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{agent.role}</span>
                    </div>
      ...
      <section id="features" className="py-24 px-6 relative bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            <h2 className="text-3xl md:text-4xl font-display font-bold text-foreground mb-4">What YieldKey does</h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <motion.div 
                ...
                variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { delay: i * 0.1 } } }}
                className="relative pl-8 before:absolute before:left-0 before:top-2 before:bottom-2 before:w-[2px] before:bg-border hover:before:bg-primary before:transition-colors"
              >
      ...
      {/* 5. HOW IT WORKS */}
      <section id="how-it-works" className="py-24 bg-muted border-y border-border px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto relative">
          <div className="text-center mb-20">
            <h2 className="text-3xl md:text-4xl font-display font-bold text-foreground mb-4">Three steps to a better deal</h2>
          </div>

          <div className="relative">
            {/* Connecting Line (Desktop) */}
            <div className="hidden lg:block absolute top-[45px] left-1/6 right-1/6 h-[2px] bg-border" />
            ...
            <div className="grid lg:grid-cols-3 gap-12 lg:gap-8 relative z-10">
              {[
                ...
                >
                  <div className="w-24 h-24 rounded-full bg-white border-2 border-border flex items-center justify-center mb-6 group-hover:border-primary group-hover:shadow-md transition-all duration-500 relative z-10">
                    <span className="font-display font-bold text-3xl text-muted-foreground group-hover:text-primary transition-colors">{item.step}</span>
                  </div>
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-2xl font-display font-bold text-foreground mb-10">Built for serious investors</h2>
          <div className="flex flex-wrap justify-center gap-3">
            {["First-time rental investors", "Airbnb hosts", "Long-term landlords", "Out-of-state buyers", "House hackers", "Small portfolio investors"].map((tag, i) => (
              <motion.span 
                ...
                className="px-4 py-2 rounded-full border border-border bg-muted text-sm font-medium text-foreground hover:bg-primary hover:text-white hover:border-primary transition-colors cursor-default"
              >
                {tag}
              </motion.span>
            ))}
      ...
      {/* 7. SECONDARY CTA */}
      <section id="cta" className="py-32 px-6 relative overflow-hidden bg-primary text-white">
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
            ...
            <motion.p variants={fadeUp} className="text-xl text-white/90 mb-12">
              Join early users shaping the smarter way to analyze rental properties.
            </motion.p>
            <motion.div variants={fadeUp} className="bg-white p-8 rounded-3xl text-left shadow-xl max-w-xl mx-auto text-foreground">
      ...
      {/* 8. FOOTER */}
      <footer className="py-12 border-t border-border bg-muted px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-4">
            <div className="w-8 h-8 rounded-full bg-white border border-border flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary cursor-pointer transition-colors shadow-sm">
        ...
        <div className="max-w-7xl mx-auto mt-8 text-center md:text-left text-xs text-muted-foreground">
